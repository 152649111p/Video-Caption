...
LOG="$CWR/logs/stage1_ce_$(date +%Y%m%d_%H%M%S).log"

# sitecustomize 让 Python 启动自动注册自定义 runner
...
python -m torch.distributed.run \
    ...
    --master_port=29501 \
    train.py \
    --cfg-path "$CWR/configs/caption_msrvtt_flant5xl_ft_ce.yaml" \
    2>&1 | tee "$LOG"

echo ""
echo "✅ 训练完成"
echo "   产物: $ROOT/02.模型训练存放（CE）/Caption_msrvtt/<job_id>/"
echo "   日志: $LOG"
