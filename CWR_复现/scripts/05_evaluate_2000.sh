...
if [ -z "$CKPT" ] || [ ! -f "$CKPT" ]; then
    echo "❌ 用法: bash 05_evaluate_2000.sh <checkpoint绝对路径>"
    exit 1
fi
...
ln -sf "$ROOT/00.数据集/msrvtt_gt/msrvtt_test_gt.json" /home/anonymous/new_ssd/lavis_datasets/msrvtt_gt/msrvtt_test_gt.json

TMP="$CWR/configs/_eval_2000_tmp_$(date +%s).yaml"
sed "s#REPLACE_BY_05_SCRIPT#$CKPT#g" "$CWR/configs/_eval_2000.yaml" > "$TMP"

LOG="$CWR/logs/eval_2000_$(date +%Y%m%d_%H%M%S).log"
