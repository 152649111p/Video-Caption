...
if [ -z "$CKPT" ] || [ ! -f "$CKPT" ]; then
    echo "❌ 用法: bash 05_evaluate_full.sh <checkpoint绝对路径>"
    exit 1
fi
...
TMP="$CWR/configs/_eval_full_tmp_$(date +%s).yaml"
sed "s#REPLACE_BY_05_SCRIPT#$CKPT#g" "$CWR/configs/_eval_full.yaml" > "$TMP"

LOG="$CWR/logs/eval_full_$(date +%Y%m%d_%H%M%S).log"
