...
if [ -z "$CKPT" ] || [ ! -f "$CKPT" ]; then
    echo "❌ 用法: bash 05_evaluate_1000.sh <checkpoint绝对路径>"
    exit 1
fi
...
TMP="$CWR/configs/_eval_1000_tmp_$(date +%s).yaml"
sed "s#REPLACE_BY_05_SCRIPT#$CKPT#g" "$CWR/configs/_eval_1000.yaml" > "$TMP"

LOG="$CWR/logs/eval_1000_$(date +%Y%m%d_%H%M%S).log"
